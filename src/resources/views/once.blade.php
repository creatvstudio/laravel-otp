...
@section('content')
	<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
		<div class="max-w-md w-full">
			...
			<form class="mt-8" action="#" method="POST" action="{{ route('otp.once.verify') }}">				
				@csrf
				...
				<div class="mt-6 flex items-center justify-between">
					<div class="flex items-center">
						<span class="ml-2 block text-sm leading-5 text-gray-900">
							<a href="{{ Session::get('url.intended', '/') }}" class="font-medium text-indigo-600 hover:text-indigo-500 focus:outline-none focus:underline transition ease-in-out duration-150">Back to Previous Page</a>
						</span>
					</div>
	...
	<form id="resend-otp-code-form" method="post" action="{{ route('otp.once.resend') }}" class="hidden">
		@csrf
	</form>
@endsection